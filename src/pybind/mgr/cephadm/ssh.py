...
class SSHManager:
    ...
    async def _execute_cephadm_exec(self,
        ...
        # Convert RemoteCommand to list of strings
        cmd_list = list(cmd_components)

        from . import serve as serve_module

        if hasattr(self.mgr, '_serve'):
            serve = self.mgr._serve
        else:
            serve = serve_module.CephadmServe(self.mgr)

        out, err, code = await serve.run_cephadm_exec(
            host=host,
            cmd=cmd_list,
            addr=addr,
            stdin=stdin,
    ...
    def execute_cephadm_exec(self,
        ...
        """
        Execute a command via cephadm exec (synchronous wrapper)
        This is the method that should be called for all regular command execution.
        """
        with self.mgr.async_timeout_handler(host, " ".join(cmd)):
    ...
    def check_execute_cephadm_exec(self,
        ...
        """Execute a command via cephadm exec and raise error if it fails (sync wrapper)"""
        with self.mgr.async_timeout_handler(host, " ".join(cmd)):
