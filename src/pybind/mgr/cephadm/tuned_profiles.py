...
class TunedProfileUtils():
    ...
    def _remove_stray_tuned_profiles(self, host: str, profiles: List[Dict[str, str]]) -> None:
        ...
        found_files = self.mgr.wait_async(
            self.mgr.ssh._check_execute_command_via_cephadm(host, cmd)
        ).split('\n')
        ...
        for file in found_files:
            ...
            if file not in expected_files:
                ...
                updated = True
        if updated:
            self.mgr.wait_async(self.mgr.ssh._check_execute_command_via_cephadm(host, SYSCTL_SYSTEM_CMD))
