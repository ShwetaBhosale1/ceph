...
def export_dict_ops_checks(cluster_id: str,
                           ...
                           qos_enable: bool,
                           qos_dict: dict) -> None:
    enable_ops_ctrl = qos_dict.get('enable_ops_control')
    if enable_ops_ctrl is None:
        return
    if not isinstance(enable_ops_ctrl, bool):
        raise Exception('Invalid values for the enable_ops_control parameter')
    ops_param_exists = any(key.endswith('count') for key in qos_dict)
    if not enable_ops_ctrl:
        if ops_param_exists:
            raise Exception('IOPS count parameters should not be passed when enable_ops_control is false.')
        return
    if enable_ops_ctrl and not ops_param_exists:
        raise Exception('IOPS count parameters should be set when enable_ops_control is true.')
    ops_obj = QOSOpsControl(enable_ops_ctrl,
                            export_ops_count=qos_dict.get(QOSParams.export_ops_count.value, 0),
                            client_ops_count=qos_dict.get(QOSParams.client_ops_count.value, 0))
...
def export_qos_ops_checks(cluster_id: str,
    ...
    if not clust_qos_obj or (clust_qos_obj and not (clust_qos_obj.enable_qos
                                                    ...
                                                    and clust_qos_obj.ops_obj.enable_ops_ctrl)):
