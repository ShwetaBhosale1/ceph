...
""".replace('{}', NFS_POOL_NAME)
    ...
    def _do_test_cluster_qos_bw(self, qos_type, combined_bw_ctrl, params, positive_tc):
        ...
        expected_out = {"enable_bw_control": True, "enable_qos": True, "combined_rw_bw_control": combined_bw_ctrl, "qos_type": qos_type.name, "enable_ops_control": False}
        for key in params:
        ...
        out = cluster.get_cluster_qos(self.cluster_id)
        assert out == {"enable_bw_control": False, "enable_qos": False, "combined_rw_bw_control": False, "enable_ops_control": False}
    ...
    def _do_test_cluster_qos_ops(self, qos_type, params, positive_tc):
        ...
        try:
            ...
            if not positive_tc:
                return
        out = cluster.get_cluster_qos(self.cluster_id)
        expected_out = {"enable_bw_control": False, "enable_qos": True, "combined_rw_bw_control": False, "qos_type": qos_type.name, "enable_ops_control": True}
        for key in params:
    ...
    @pytest.mark.parametrize("qos_type, params, positive_tc", [
        (QOSType['PerShare'], {'export_ops_count': 10000}, True),
        (QOSType['PerClient'], {'client_ops_count': 20000}, True),
        (QOSType['PerShare_PerClient'], {'export_ops_count': 3000, 'client_ops_count': 40000}, True),
        # negative testing
        (QOSType['PerShare_PerClient'], {'export_ops_count': 1000}, False),
        (QOSType['PerShare'], {'client_ops_count': 10000}, False),
        (QOSType['PerShare'], {}, False),
        (QOSType['PerClient'], {'export_ops_count': 2000, 'client_ops_count': 1000}, False),
        (QOSType['PerShare_PerClient'], {'export_ops_count': 10}, False)
        ])
    ...
    def _do_test_export_qos_ops(self, qos_type, clust_params, export_params):
        ...
        try:
            ...
            if qos_type.name == 'PerClient':
                return
        out = export_mgr.get_export_qos(self.cluster_id, '/cephfs_a/')
        expected_out = {"enable_ops_control": True, "enable_qos": True}
        for key in export_params:
            expected_out[QOSParams[key].value] = export_params[key]
        assert out == expected_out
        export_mgr.disable_export_qos_ops(self.cluster_id, '/cephfs_a/')
        out = export_mgr.get_export_qos(self.cluster_id, '/cephfs_a/')
        assert out == {"enable_ops_control": False, "enable_qos": False}

    @pytest.mark.parametrize("qos_type, clust_params", [
        (QOSType['PerShare'], {'export_ops_count': 10000}),
        (QOSType['PerClient'], {'client_ops_count': 20000}),
        (QOSType['PerShare_PerClient'], {'export_ops_count': 3000, 'client_ops_count': 40000})
        ])
    @pytest.mark.parametrize("export_params", [
        ({'export_ops_count': 10000}),
        ({'client_ops_count': 20000}),
        ({'export_ops_count': 3000, 'client_ops_count': 40000})
        ])
    ...
    def _do_test_cluster_qos_bw_ops(self, bw_qos_type, bw_params, ops_qos_type, ops_params, positive_tc):
        ...
        out = cluster.get_cluster_qos(self.cluster_id)
        expected_out = {"enable_bw_control": True, "enable_qos": True, "combined_rw_bw_control": False, "qos_type": ops_qos_type.name, "enable_ops_control": True}
        bw_out = {}
        ops_out = {}
        ...
        cluster.disable_cluster_qos_bw(self.cluster_id)
        out = cluster.get_cluster_qos(self.cluster_id)
        ops_out.update({"enable_bw_control": False, "enable_qos": True, "combined_rw_bw_control": False, "enable_ops_control": True, "qos_type": ops_qos_type.name})
        assert out == ops_out
        # disable ops control
        cluster.disable_cluster_qos_ops(self.cluster_id)
        out = cluster.get_cluster_qos(self.cluster_id)
    ...
    @pytest.mark.parametrize("bw_qos_type, bw_params, ops_qos_type, ops_params, positive_tc", [
        ...
        (QOSType['PerShare_PerClient'], {'export_writebw': '100MB', 'export_readbw': '200MB', 'client_writebw': '300MB', 'client_readbw': '400MB'}, QOSType['PerClient'], {'client_ops_count': 20000}, False),
        ])
    ...
    def _do_test_export_qos_bw_ops(self, qos_type, clust_bw_params, clust_ops_params, export_bw_params, export_ops_params):
        ...
        try:
            ...
            if sorted(export_ops_params.keys()) != sorted(req):
                return
        out = export_mgr.get_export_qos(self.cluster_id, '/cephfs_a/')
        expected_out = {"enable_bw_control": True, "enable_qos": True, "combined_rw_bw_control": False, "enable_ops_control": True}
        bw_out = {}
        ops_out = {}
        ...
        export_mgr.disable_export_qos_bw(self.cluster_id, '/cephfs_a/')
        out = export_mgr.get_export_qos(self.cluster_id, '/cephfs_a/')
        ops_out.update({"enable_bw_control": False, "enable_qos": True, "combined_rw_bw_control": False, "enable_ops_control": True})
        assert out == ops_out
    ...
    @pytest.mark.parametrize("qos_type, clust_bw_params, clust_ops_params", [
        # positive TCs
        (QOSType['PerShare'], {'export_writebw': '100MB', 'export_readbw': '200MB'}, {'export_ops_count': 10000}),
        (QOSType['PerClient'], {'client_writebw': '300MB', 'client_readbw': '400MB'}, {'client_ops_count': 20000}),
        (QOSType['PerShare_PerClient'],
         {'export_writebw': '100MB', 'export_readbw': '200MB', 'client_writebw': '300MB', 'client_readbw': '400MB'}, {'export_ops_count': 3000, 'client_ops_count': 40000})
    ])
    @pytest.mark.parametrize("export_bw_params, export_ops_params", [
        ({'export_writebw': '100MB', 'export_readbw': '200MB'}, {'export_ops_count': 10000}),
        ({'client_writebw': '300MB', 'client_readbw': '400MB'}, {'client_ops_count': 20000}),
        ({'export_writebw': '100MB', 'export_readbw': '200MB', 'client_writebw': '300MB', 'client_readbw': '400MB'}, {'export_ops_count': 3000, 'client_ops_count': 40000})
        ])
