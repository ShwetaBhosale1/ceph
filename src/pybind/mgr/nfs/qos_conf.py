...
class QOSParams(Enum):
    ...
    # ops control
    enable_ops_ctrl = "enable_ops_control"
    export_ops_count = "export_ops_count"
    client_ops_count = "client_ops_count"
...
QOS_REQ_OPS_PARAMS = {
    'PerShare': ['export_ops_count'],
    'PerClient': ['client_ops_count'],
    'PerShare_PerClient': ['export_ops_count', 'client_ops_count']
}
...
class QOSOpsControl(object):
    def __init__(self,
                 enable_ops_ctrl: bool = False,
                 export_ops_count: int = 0,
                 client_ops_count: int = 0
                 ) -> None:
        self.enable_ops_ctrl = enable_ops_ctrl
        self.export_ops_count = _validate_qos_ops(export_ops_count)
        self.client_ops_count = _validate_qos_ops(client_ops_count)

    @classmethod
    def from_dict(cls, qos_dict: Dict[str, Any]) -> 'QOSOpsControl':
        kwargs: dict[str, Any] = {}
        kwargs['enable_ops_ctrl'] = qos_dict.get(QOSParams.enable_ops_ctrl.value, False)
        kwargs['export_ops_count'] = qos_dict.get(QOSParams.export_ops_count.value, 0)
        kwargs['client_ops_count'] = qos_dict.get(QOSParams.client_ops_count.value, 0)
        return cls(**kwargs)

    @classmethod
    def from_qos_block(cls, qos_block: RawBlock) -> 'QOSOpsControl':
        kwargs: dict[str, Any] = {}
        kwargs['enable_ops_ctrl'] = qos_block.values.get(QOSParams.enable_ops_ctrl.value, False)
        kwargs['export_ops_count'] = qos_block.values.get(QOSParams.export_ops_count.value, 0)
        kwargs['client_ops_count'] = qos_block.values.get(QOSParams.client_ops_count.value, 0)
        return cls(**kwargs)

    def to_qos_block(self) -> RawBlock:
        result = RawBlock('qos_ops_control')
        result.values[QOSParams.enable_ops_ctrl.value] = self.enable_ops_ctrl
        if self.export_ops_count:
            result.values[QOSParams.export_ops_count.value] = self.export_ops_count
        if self.client_ops_count:
            result.values[QOSParams.client_ops_count.value] = self.client_ops_count
        return result

    def to_dict(self) -> Dict[str, Any]:
        r: dict[str, Any] = {}
        r[QOSParams.enable_ops_ctrl.value] = self.enable_ops_ctrl
        if self.export_ops_count:
            r[QOSParams.export_ops_count.value] = self.export_ops_count
        if self.client_ops_count:
            r[QOSParams.client_ops_count.value] = self.client_ops_count
        return r
...
class QOS(object):
    ...
    def get_enable_qos_val(self, disable_bw: bool = False, disable_ops: bool = False) -> bool:
        ...
        if disable_bw and self.ops_obj and self.ops_obj.enable_ops_ctrl:
            return True
