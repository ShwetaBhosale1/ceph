...
def main() -> int:
    ...
    if not validate_cephadm_path(cephadm_path):
        ...
        # This matches the convention used by cephadm itself and triggers deployment
        return 2
    ...
    if not verify_cephadm_hash(cephadm_path):
        ...
        print("Switching to restricted mode:", file=sys.stderr)
        print("  - Bootstrap commands (curl, wget, mkdir, etc.) → execute directly", file=sys.stderr)
        print("  - Other commands (ls, systemctl, etc.) → use 'cephadm exec'", file=sys.stderr)
        # In restricted mode, pass cephadm_path so non-bootstrap commands can use exec
        return execute_direct_command(cephadm_args, cephadm_path=cephadm_path)
